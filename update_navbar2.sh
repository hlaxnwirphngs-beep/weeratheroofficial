cat << 'INNER_EOF' > src/components/layout/Navbar.tsx
    ...
    <nav className="fixed w-full z-50 bg-[#E4E2DA] border-b border-sep transition-all">
        ...
        <div className="flex justify-between items-center h-20">
          <Link to="/" className="flex items-center gap-3 sm:gap-4 min-w-0 pr-4">
            ...
            <div className="flex flex-col min-w-0">
              <span className="font-bold text-xs sm:text-sm lg:text-base tracking-widest text-accent-deep truncate">
                {siteTitle}
              </span>
              <span className="text-[8px] sm:text-[10px] uppercase opacity-60 tracking-[0.2em] hidden sm:block truncate">
      ...
      {isOpen && (
        <motion.div
          ...
          className="md:hidden bg-[#E4E2DA] border-t border-sep/50 shadow-xl"
        >
